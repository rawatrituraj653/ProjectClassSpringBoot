...
public class UserController {
	...
	public String getUserData(@ModelAttribute("user") User user, Model model) {
		...
		if (prId != null) {
			model.addAttribute(AppConstants.USER_MOD_OBj, new User());
			String sucmsg = props.getMessages().get(AppConstants.SUCC_MSG);
			model.addAttribute(AppConstants.MODEL_KEY, sucmsg + "  "+prId);
		} else {
			String failmsg = props.getMessages().get(AppConstants.FAIL_MSG);
			model.addAttribute(AppConstants.MODEL_KEY, failmsg);
		}
		loadFormData(model);
		return AppConstants.LOGIC_VIEW_NAME_REG;
	}
	...
	public @ResponseBody String findUserById(@RequestParam("userId") Integer userId,RedirectAttributes model) throws JsonProcessingException {
		User user = service.findUserById(userId);
